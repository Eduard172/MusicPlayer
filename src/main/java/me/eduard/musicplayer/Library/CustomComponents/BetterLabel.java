...
import me.eduard.musicplayer.Utils.Coords2D;
...
public final class BetterLabel {
    ...
    public double getLabelWidth() {
        return this.label1.getPrefWidth();
    }
    public double getLabelHeight() {
        return this.label1.getPrefHeight();
    }
    ...
    private void updateText(String newText){
        Platform.runLater(() -> {
            ...
            this.labelProperty = this.label1.textProperty();
            if(this.label1.getText().equals(newText))
                return;
            this.startAnimation(this.label1, true, true, 1);
        });
    ...
    private Coords2D getExactLabel_X_Coords(Label label){
        Coords2D coords2D = new Coords2D();
        coords2D.setMinX(label.translateXProperty().get());
        coords2D.setMaxX(label.translateXProperty().get() + label.getWidth());
        return coords2D;
    }
    ...
    private void moveToRight(Label label){
        Platform.runLater(() -> label.translateXProperty().set(this.scrollPane.getPrefWidth() + 3));
    }
    ...
    private void startAnimation(Label label, boolean stopCurrent, boolean centralize, int labelNo){
        Runnable runnable = () -> {
            ...
            WrappedValue<Boolean> stackUpCheck = WrappedValue.of(true);
            WrappedValue<Boolean> reachedPointCheck = WrappedValue.of(true);
                ...
                protected Void call(){
                    animationTimer = new AnimationTimer() {
                        ...
                        public void handle(long now) { //the 'now' var is in nanos.
                            Coords2D exactPosition = getExactLabel_X_Coords(label);
                            if(stackUpCheck.get() && ((exactPosition.getMaxX() + relativeLabelDistance) <= scrollPane.getPrefWidth())){
                                ...
                                if(b){ //Means Label 1
                                    ...
                                    keepMoving = new AnimationTimer() {
                                        @Override
                                        public void handle(long now) {
                                            Coords2D prevPos = getExactLabel_X_Coords(label1);
                                    ...
                                    keepMoving = new AnimationTimer() {
                                        @Override
                                        public void handle(long now) {
                                            Coords2D prevPos = getExactLabel_X_Coords(label2);
                                            if(prevPos.getMaxX() <= 0){
        ...
        Utilities.sleep(Duration.millis(2), 1, run -> Platform.runLater(runnable), null);
    }
