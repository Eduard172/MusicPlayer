...
public class Animations {
    ...
    private static final Logger LOGGER = Logger.getLogger("App-Animations");

    /**
     * Initialized inside {@link Player#initialize}
     */
    public static BasicAnimator playerFullScreenAnimate;
    ...
    public static void undoAnimateObjectAppear(Node node, int millisSpeed, boolean makeInvisible){
        checkAndStopAnimations(disappearTimeline, appearTimeLine);
        disappearTimeline = Utilities.sleep(Duration.millis(millisSpeed), (int) (1.0 / 0.05),
                run -> {
                    if(node.getOpacity() == 0){
                        node.setVisible(!makeInvisible);
                        disappearTimeline.stop();
                    }
                    node.setOpacity((node.getOpacity() - 0.05 < 0) ? 0 : node.getOpacity() - 0.05);
                },
                null
        );
    }
    ...
    public static void animateObjectAppear(Node node, int millisSpeed, double maxOpacity){
        WrappedValue<Double> opacity = WrappedValue.of(maxOpacity);
        if(opacity.get() < 0 || opacity.get() > 1){
            opacity.set(0.5);
            LOGGER.warning("Max opacity value is out of bounds (0 - 1). It was changed to 0.5");
        }
        checkAndStopAnimations(appearTimeLine, disappearTimeline);
        node.setVisible(true);
        node.setOpacity(0.0);
        appearTimeLine = Utilities.sleep(Duration.millis(millisSpeed), (int) (1.0 / 0.05),
                run -> {
                    if(node.getOpacity() >= maxOpacity) appearTimeLine.stop();
                    node.setOpacity(Math.min(node.getOpacity() + 0.05, 1.0));
            }, null
        );
    }
