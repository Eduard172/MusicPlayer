...
public final class FilesUtils {
    ...
    public static boolean moveFile(File source, String path){
            ...
            while (entry != null) {
                ...
                for(String s : specificFilesName){
                    if (filePath.endsWith(s)){
                        ...
                        extract(zipIn, parts[parts.length - 1]);
                    }
                }
                zipIn.closeEntry();
    ...
    public static void removeNonPlaylistFiles(String playlist){
        ...
        final String thumbnailName = "Thumbnail."+Playlists.thumbnailExt;
    ...
    public static void playAudioFileIndependently(String path){
        File file = new File(path);
        if(!file.exists())
            return;
        try {
