...
import me.eduard.musicplayer.Library.AppFlags;
...
public class FXMLStageBuilder {
    ...
    private final SimplePair<EventType<MouseEvent>, EventHandler<? super MouseEvent>> mouseEvent = SimplePair.of(MouseEvent.MOUSE_MOVED, event -> {
        AppFlags.isMouseOverMainStage = false;
    ...
    public final FXMLStageBuilder bindAllStagesCloseKeyCombination(){
        this.addKeyEvents(
                ...
                SimplePair.of(KeyEvent.KEY_RELEASED, event -> KeyCombinationUtils.removeKey(event.getCode()))
        );
        return this;
    }

    public final FXMLStageBuilder requireModality(){
        this.stageBuilder.getStage().initOwner(MainApp.stage1);
        this.stageBuilder.getStage().initModality(Modality.APPLICATION_MODAL);
        this.isModal = true;
        return this;
    }
    ...
    @SafeVarargs
    public final void addKeyEvents(SimplePair<EventType<KeyEvent>, EventHandler<? super KeyEvent>>... basicKeyValuePairs){
        this.throwOperationFailedIf(this.stageBuilder == null);
        for(SimplePair<EventType<KeyEvent>, EventHandler<? super KeyEvent>> pair : basicKeyValuePairs){
            this.stageBuilder.getStage().getScene().addEventFilter(pair.getKey(), pair.getValue());
        }
    }
    @SafeVarargs
    public final void addMouseEvents(SimplePair<EventType<MouseEvent>, EventHandler<? super MouseEvent>>... basicKeyValuePairs){
        this.throwOperationFailedIf(this.stageBuilder == null);
        for(SimplePair<EventType<MouseEvent>, EventHandler<? super MouseEvent>> eachPair : basicKeyValuePairs){
