...
public class RemoveSongs extends OutputUtilities {
    ...
    public static void launchWindow(String selectedPlaylist){
        try {
            ...
            WindowTitleBar titleBar = new WindowTitleBar(stage).useDefaultPresets()
                    .setTitleString("Remove songs from '"+Player.SELECTED_PLAYLIST+"' playlist.")
            ...
            GlobalAppStyle.applyToButtons(removeSongs.cancelButton, removeSongs.proceedButton, removeSongs.removeButton);
            GlobalAppStyle.applyToCheckbox(removeSongs.instantRemove);

            removeSongs.initializeListView(selectedPlaylist);
            removeSongs.cancelButton.setOnAction(event -> MainApp.closeStage(stage, Player.ANIMATIONS));
