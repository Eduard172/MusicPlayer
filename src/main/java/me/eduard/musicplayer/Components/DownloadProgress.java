...
public class DownloadProgress {
    ...
    @FXML public Label currentlyEncoding;

    private int stepsFinished = 0;
    private int totalSteps = 1;
    private int downloadedFiles = 0;
    private int totalFilesToDownload = 1;
    private String currentlyEncodingElement = "Nothing at the moment.";
    ...
    private final String CURRENTLY_ENCODING = "[current]";
    ...
    private String currentlyEncodingText = "Currently Encoding: [current]";
    ...
    public void setCurrentlyEncoding(String current){
        this.currentlyEncodingElement = current == null ? "Nothing at the moment." : current;
        this.currentlyEncoding.setText(currentlyEncodingText
                .replace(CURRENTLY_ENCODING, this.currentlyEncodingElement)
        );
    }
    ...
    public void close(DownloadProgress instance){
        MainApp.closeStage(instance.getStage(), Player.ANIMATIONS);
    }
    ...
    private DownloadProgress launch(){
        try {
            ...
            GlobalAppStyle.applyToButtons(downloadProgress.cancelButton);
            downloadProgress.cancelButton.setOnAction(event -> {
                downloadProgress.cancelButton.setVisible(false);
                downloadProgress.setStatusInProgress("Cleaning up...");
                DataStructures.PROCESSES.forEach(Process::destroyForcibly);
                DataStructures.PROCESSES.clear();
                FilesUtils.removeNonPlaylistFiles(Player.SELECTED_PLAYLIST);
                AppMode.set(AppState.NORMAL);
                Utilities.sleep(Duration.millis(150), 1, run -> stage.close(), null);
            });

            WindowTitleBar windowTitleBar = new WindowTitleBar(stage).useDefaultPresets()
                    .setButtonsArrangement(WindowTitleBar.ButtonsArrangement.NONE)
                    .setSupportStyling(MainApp.childWindowTitleBarTheme)
                    .setTitleLabelStyling("-fx-text-fill: #000000")
                    .setTitleBarImage(null)
                    .setTitleString("Download in progress...");
            windowTitleBar.linkToStage();

            MainApp.openStage(stage, Player.ANIMATIONS);
            downloadProgress.currentlyEncoding.requestFocus();
