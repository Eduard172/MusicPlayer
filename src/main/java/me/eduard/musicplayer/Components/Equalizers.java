...
import me.eduard.musicplayer.Library.MessageDialog;
...
public class Equalizers implements Initializable {
    ...
    @FXML public Slider slider32, slider64, slider125, slider250, slider500, slider1000, slider2000, slider4000, slider8000, slider16000;
    @FXML public Spinner<Double> field32, field64, field125, field250, field500, field1000, field2000, field4000, field8000, field16000;
    @FXML public Hyperlink reset32, reset64, reset125, reset250, reset500, reset1000, reset2000, reset4000, reset8000, reset16000;
    @FXML public Button resetToDefault, turnAllMax, turnAllMin;
    @FXML public CheckBox enableEq;

    private static final double MIN_GAIN = -12;
    private static final double MAX_GAIN = 12;
    ...
    private void setupSpinnerValueFactory(Spinner<Double> spinner, Slider slider){
        spinner.setValueFactory(new SpinnerValueFactory.DoubleSpinnerValueFactory(
                -12, 12, Utilities.getValueOfFixedLimits(MIN_GAIN, MAX_GAIN, slider.getValue()), 0.01)
        );
    }
    ...
    private SimplePair<Spinner<Double>, Slider> getSpinnerSliderPairBasedOnIndex(int index){
        return switch (index){
            case 0 -> SimplePair.of(this.field32, this.slider32);
            case 1 -> SimplePair.of(this.field64, this.slider64);
            case 2 -> SimplePair.of(this.field125, this.slider125);
            case 3 -> SimplePair.of(this.field250, this.slider250);
            case 4 -> SimplePair.of(this.field500, this.slider500);
            case 5 -> SimplePair.of(this.field1000, this.slider1000);
            case 6 -> SimplePair.of(this.field2000, this.slider2000);
            case 7 -> SimplePair.of(this.field4000, this.slider4000);
            case 8 -> SimplePair.of(this.field8000, this.slider8000);
            case 9 -> SimplePair.of(this.field16000, this.slider16000);
            default -> throw new IllegalArgumentException("Index range should be between 0 and 10.");
        };
    }
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        equalizers.setDefaultSettings(SETTING_VALUES);
        NodeUtils.setTooltip(enableEq, "Instantly disable / enable the equalizer", 200);
        enableEq.setSelected(Boolean.parseBoolean(equalizers.getSettingValue("Enabled", false)));
        enableEq.selectedProperty().addListener((obs, oldVal, newVal) -> {
            Player player = Player.instance;
            if(player.audioPlayer != null){
                player.audioPlayer.getAudioEqualizer().setEnabled(newVal);
                LOGGER.info("Equalizer has been "+(enableEq.isSelected() ? "enabled" : "disabled"));
            }
            equalizers.saveSetting("Enabled", newVal);
        });
        GlobalAppStyle.applyToCheckbox(enableEq);
    ...
    private void initializeBasicComponents(){
        ...
        this.turnAllMax.setOnAction(event -> this.turnGainsToExtremity(TurnType.MAX, this, false));
        this.turnAllMin.setOnAction(event -> this.turnGainsToExtremity(TurnType.MIN, this, true));
    ...
    private void resetValuesToDefault(Equalizers instance){
        ...
        for(int i = 0; i < EQUALIZERS.length; i++){
            equalizers.setupSettingsFile(true, SETTING_VALUES);
            equalizers.saveSetting("Enabled", enableEq.isSelected());
            player.audioPlayer.getAudioEqualizer().getBands().get(i).setGain(0);
            instance.getSpinnerSliderPairBasedOnIndex(i).getValue().setValue(
                    Utilities.getPercentageOfFixedLimits(MIN_GAIN, MAX_GAIN, 0)
            );
        }
    }

    public void turnGainsToExtremity(TurnType turnType, Equalizers instance, boolean ignoreWarning){
        Player player = Player.instance;
        if(player.audioPlayer == null){
            return;
        }
        if(turnType == TurnType.MAX && !ignoreWarning){
            MessageDialog.newMessageDialog()
                    .setTitle("Wait a second")
                    .setMessage("""
                            Turning all frequencies to max volume at once rather than turning them manually may cause ear shocks. \
                            Are you sure you want to continue?
                            """)
                    .setAnswerType(MessageDialog.AnswerType.YES_NO)
                    .setLevel(MessageDialog.Level.WARNING)
                    .setOnYes(event -> this.turnGainsToExtremity(turnType, instance, true))
                    .setOnNo(null)
                    .launchDialog();
        }
        if(ignoreWarning){
    ...
    private void launch(){
        try {
            ...
            GlobalAppStyle.applyToButtons(
                    equalizers.resetToDefault,
                    equalizers.turnAllMax,
                    equalizers.turnAllMin
            );
            GlobalAppStyle.applyToHyperLinks(
                    equalizers.reset32,
                    equalizers.reset64,
                    equalizers.reset125,
                    equalizers.reset250,
                    equalizers.reset500,
                    equalizers.reset1000,
                    equalizers.reset2000,
                    equalizers.reset4000,
                    equalizers.reset8000,
                    equalizers.reset16000
            );
    ...
    public static EqualizerBand[] getEqualizers() {
        if (!LOADED) {
            //Q Factor: 1.6, default: 1.6
            //Old Frequencies: 32, 64, 125, 250, 500, 1000, 2000, 4000, 8000, 16000
            //Old Bandwidths: 19, 39, 78, 156, 312, 625, 1250, 2500, 5000, 10000
            //Bandwidth formula: CenterFrequency / Q
            EQUALIZERS = new EqualizerBand[]{
                    new EqualizerBand(32, 20, Double.parseDouble(equalizers.getSettingValue("freq32", false))),
                    new EqualizerBand(64, 40, Double.parseDouble(equalizers.getSettingValue("freq64", false))),
                    new EqualizerBand(125, 80, Double.parseDouble(equalizers.getSettingValue("freq125", false))),
                    new EqualizerBand(250, 170, Double.parseDouble(equalizers.getSettingValue("freq250", false))),
                    new EqualizerBand(500, 312, Double.parseDouble(equalizers.getSettingValue("freq500", false))),
                    new EqualizerBand(1000, 625, Double.parseDouble(equalizers.getSettingValue("freq1000", false))),
                    new EqualizerBand(2000, 1250, Double.parseDouble(equalizers.getSettingValue("freq2000", false))),
                    new EqualizerBand(4000, 2500, Double.parseDouble(equalizers.getSettingValue("freq4000", false))),
                    new EqualizerBand(8000, 5000, Double.parseDouble(equalizers.getSettingValue("freq8000", false))),
                    new EqualizerBand(16000, 10000, Double.parseDouble(equalizers.getSettingValue("freq16000", false)))
            };
            LOADED = true;
            LOGGER.info("Loaded "+EQUALIZERS.length+" bands gain from 'Equalizers.yml'.");
        }
        return EQUALIZERS;
    }

    @SuppressWarnings("unused")
    public static void generateTestingEqualizersCode(){
        System.out.println("Equalizers.testEqualizers(");
        for(EqualizerBand band : EQUALIZERS){
            System.out.println("\tSimplePair.of("+band.getCenterFrequency()+", "+band.getBandwidth()+"),");
        }
        System.out.println(");");
    }

    @SafeVarargs
    public static void test(SimplePair<Number, Number>... centerAndBandwidths){
        if(centerAndBandwidths.length > 64){
            LOGGER.severe("You have more than 64 equalizers set.");
            return;
        }
        for(SimplePair<Number, Number> pair : centerAndBandwidths){
            double summed = pair.getKey().doubleValue() + (pair.getValue().doubleValue() / 2);
            double subtracted = pair.getKey().doubleValue() - (pair.getValue().doubleValue() / 2);
            LOGGER.info("For Center Freq = "+pair.getKey()+" range is: "+subtracted+" ... "+summed);
        }
    }
}
