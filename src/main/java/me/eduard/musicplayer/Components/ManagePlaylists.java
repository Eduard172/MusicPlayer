...
public class ManagePlaylists extends OutputUtilities implements Initializable {
    ...
    public void proceedToAction(Actions actions, String selectedPlaylist){
        ...
        switch (actions){
            ...
            case CLEANUP_PLAYLIST -> {
                ScreenLauncher.launchPlaylistCleanupScreen(selectedPlaylist);
                if(Playlists.getNonPlaylistFiles(selectedPlaylist) > 0){
                }else{
                    this.setStatusLabel("There are no files to cleanup for this playlist.", Level.INCOMPLETE, Player.ANIMATIONS);
                }
            }
        }
    }
    ...
    public static void launchWindow(){
        try {
            ...
            WindowTitleBar titleBar = new WindowTitleBar(stage).useDefaultPresets()
                    .setTitleString("Manage your playlists.")
            ...
            GlobalAppStyle.applyToButtons(managePlaylists.cancelButton, managePlaylists.proceedToActionButton);
