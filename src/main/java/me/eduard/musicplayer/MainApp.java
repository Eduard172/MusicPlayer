...
import me.eduard.musicplayer.Components.Player.PlayerUtils;
...
@SuppressWarnings({"unused", "ConstantValue", "SpellCheckingInspection"})
public class MainApp extends Application{
    ...
    public static final MessageDialog messageDialog = MessageDialog.newMessageDialog();
    ...
    public WindowTitleBar playerTitleBar = null;
    ...
    public void start(Stage stage) throws Exception{
        ...
        instance = this;
//        FFmpegUtils.downloadZipFileIfNecessary(null);
    ...
    public static final String FFMPEG = APP_EXTERNAL_HELPERS+"\\ffmpeg.exe";
    public static final String OPUS_DECODER = APP_EXTERNAL_HELPERS+"\\opusdec.exe";
    public static final String DOWNLOADER = APP_EXTERNAL_HELPERS+"\\downloader.exe";
    public static final List<Stage> STAGES = new ArrayList<>();
    public static Stage stage1;
    public static Scene scene;
    public static Pane parent;
    public static final String mainWindowTitleBarTheme = "-fx-fill: linear-gradient(to right, #00c9c3, #018f8b)"; //#00c9c3
    public static final String childWindowTitleBarTheme = "-fx-fill: #00ada8";
    ...
    public void setupComponents(){
        ...
        if(!mainDirectory.exists() && !mainDirectory.getPath().contains(".")){
            ...
            new VersionHandler().manageIntegrity();
            if(AppMode.get() == AppState.WAITING_FOR_UPDATE_APPROVAL) {
                return;
            }
    ...
    public static void quitApp(boolean force, AppState applicationState){
        if(force || applicationState == AppState.NORMAL){
            ...
            Caches.clearCaches();
            DataStructures.PROCESSES.forEach(Process::destroy);
            player.FAILED_MEDIAS.forEach(MediaPlayer::dispose);
            LOGGER.info("All failed video / audio players were disposed.");
            executorService.shutdown();
            player.THREADS.forEach(ExecutorService::shutdownNow);
    ...
    public void startApp() {
        try {
            ...
            stage1 = StageBuilder.newBuilder()
                    ...
                    .resizable(false)
                    .buildAndGet();
            this.playerTitleBar = new WindowTitleBar(stage1).useDefaultPresets()
                            .setTitleString(title)
                            .setOnFSPTitleString("Toggle FullScreen")
                            .setSupportStyling(mainWindowTitleBarTheme)
                            .setOnClose(() -> {
                                LOGGER.severe("CLOSING");
                                quitApp(false, this.getApplicationState());
                            })
                            .setButtonsArrangement(WindowTitleBar.ButtonsArrangement.EVERYTHING)
                            .setViewMode(WindowTitleBar.ViewMode.ABOVE)
                            .setOnFullScreenEnable(FullScreenMode.enableFullScreen())
                            .setTitleLabelStyling("-fx-text-fill: #000000")
                            .setOnFSPSupportStyling("-fx-fill: #00fff7")
                            .setOnFSPTitleStyling("-fx-text-fill: #000000", "-fx-font-weight: bold")
                            .setFullScreenWarnText("Release the mouse to enter fullscreen")
                            .setFullScreenWarnLabelStyling("-fx-text-fill: #00fff7", "-fx-alignment: CENTER")
                            .setFullScreenWarnFont(40)
                            .setOnFullScreenPrepare(() -> Utilities.runAsynchronously(() -> {
                                Animations.playerFullScreenAnimate.stop();
                                Animations.playerFullScreenAnimate.markToDisappear(false).setMillis(30).start();
                            }))
                            .setOnFullScreenPrepareExit(() -> Utilities.runAsynchronously(() -> {
                                Animations.playerFullScreenAnimate.stop();
                                Animations.playerFullScreenAnimate.markToDisappear(true).setMillis(15).start();
                            }))
                            .setOnFullScreenDisable(FullScreenMode.disableFullScreen());
            this.playerTitleBar.linkToStage();
            ...
            if (this.firstTimeStart) {
                WindowsCommands.launchPowershellNotification(
                        ...
                        """
                                ...
                                create your very first playlist. I hope you'll enjoy using this App. :)
            ...
            try {
                if(Playlists.getPlaylistsCount() > 0){
                    LOGGER.info("Trying to automatically clean up non playlist files...");
                    FilesUtils.removeNonPlaylistFiles(Player.SELECTED_PLAYLIST);
                }
            }catch (NullPointerException ex){
                LOGGER.warning("Cannot clean up automatically this playlist. ("+ex.getClass()+")");
            }
    ...
    private void initializeSceneKeyBinds(Scene scene){
        ...
        if(player.devLabel.isVisible()){
            scene.addEventHandler(MouseEvent.MOUSE_ENTERED, event -> {
                ...
                AppFlags.isMouseOverMainStage = true;
                Animations.undoAnimateOverAllScreen(player, false);
            });
        }else{
            scene.addEventFilter(MouseEvent.MOUSE_MOVED, event -> AppFlags.isMouseOverMainStage = true);
        }
        scene.addEventFilter(KeyEvent.KEY_PRESSED, event -> {
            if(player.searchSong.isFocused()){
                return;
            }
            player.dummyLabel.requestFocus();
            ...
            if(KeyCombinationUtils.isKey(KeyCode.ALT, 0) && KeyCombinationUtils.isKey(KeyCode.S, 1)){
                ...
                if(Player.fullScreenMode.isFullScreen) {
                    Player.fullScreenMode.disableFullScreen();
                }else{
                ...
                if(playerTitleBar.isFullScreen()){
                    playerTitleBar.disableFullScreen();
                }else{
                    playerTitleBar.enableFullScreen();
                }
                ...
                ScreenLauncher.launchTestingWindow();
            }
            ...
            if(player.audioPlayer != null){
                ...
                if(KeyCombinationUtils.isKey(KeyCode.CONTROL, 0) && KeyCombinationUtils.isKey(KeyCode.F, 1)){
                    ...
                    LabelAnimations.instance(player.statusLabel)
                            .waitingTime(1000).text("Playing from beginning...").color(Color.LIME)
                            .startAnimation();
                }
    ...
    @SuppressWarnings("all") public static void startAutomaticGarbageCollectorCall(int duration){
        ...
        executorService.submit(() -> {
            ...
            Utilities.sleep(Duration.millis(1000), -1,
                    event -> {
                        ...
                        if(Player.VERBOSE_STATUS){
                            player.memoryCleanup.setText((remaining > 0) ?
                                    "Automatic resource free up in "+timer+"..." : "The App has freed as many resources as it could.");
                            if(player.audioPlayer != null && !Player.HIDE_PLAYER){
                                double diff = PlayerUtils.getAVDifference();
                                player.avSyncIndicator.setText(String.format(
                                        "A/V sync delay: %dms", Math.abs((int) diff)));
                                player.avSyncIndicator.setTextFill(diff > 5000 ? Color.RED : diff > 250 ? Color.ORANGE : Color.LIME);
                                if(diff < 60) {
                                    AppFlags.videoSeeks.set(0);
                                }
                                if((diff > 60 || Math.abs(diff) > 100) && AppFlags.videoSeeks.get() < 15) {
                                    Player.LOGGER.warning("Calibrating video player (Difference of "+((int) diff)+"ms)");
                                    player.videoPlayer.seek(player.audioPlayer.getCurrentTime().add(Duration.millis(60)));
                                    AppFlags.videoSeeks.set(AppFlags.videoSeeks.get() + 1);
                                    if(AppFlags.videoSeeks.get() >= 15) {
                                        Player.LOGGER.warning("Tried seeking 15 times in a row. Leaving the video as is.");
                                    }
                                }
                            }
                        }
                        if(remaining <= 0){
                            seconds.set(0);
                            Caches.clearCaches();
                            Utilities.stopAllFailedMediaPlayers();
    ...
    public static void uninstallApp(int attempt){
        if(attempt == 25){
            WindowsCommands.launchPowershellNotification(
                    "Uninstallation is struggling...",
                    "Would you like to check if you have some open apps using the main directory?"
            );
        }else if(attempt == 45){
            WindowsCommands.launchPowershellNotification(
                    "Uninstallation is REALLY struggling...",
                    "Now for real, check if you have any opened apps that are using the main 'MusicPlayer' folder on your desktop"
            );
        }else if(attempt == 70){
            WindowsCommands.launchPowershellNotification(
                    "Uninstallation IS STILL struggling...",
                    """
                            The app will try a few more times to uninstall. If it still fails, it will automatically quit and open the main directory \
                            for you, so you can remove the files manually
                            """
            );
        }else if(attempt == 100){
            WindowsCommands.launchPowershellNotification(
                    ...
                    """
                            The app has been closed due to 100 failed uninstallation attempts. The main directory has been opened \
                            so you can remove it manually from your PC.
                            """
            );
            Utilities.openApplicationDirectory();
            quitApp(true, instance.getApplicationState());
        }
        Player player = Player.instance;
